...
from app.db.models import (
    DataCollectionPriority, MissingDataItem, CourseDecision, Ders, Fakülte, Bolum
)
    ...
    try:
        ...
        for course_id, data in sorted_courses:
            ...
            if data['critical_count'] >= 2:
                ...
                impact = "medium"
                reason = f"En az 1 kritik veri eksik"
            else:
                impact = "low"
                ...
                for dept in session.query(Bolum).filter(Bolum.fakulte_id == fak.fakulte_id):
                    ...
                    if low_priority > (session.query(Ders).filter(Ders.bolum_id == dept.bolum_id).count() / 2):
                        priority = DataCollectionPriority(
                            ...
                            suggested_action=f"Bölüm başkanı ile iletişim: Tüm dersler için veri toplanmalı.",
                            status="open",
                            created_at=_now(),
                        )
                        session.add(priority)
